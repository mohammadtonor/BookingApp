...
const Reserve = ({setOpen, hotelId}) => {
  ...
  const isAvailable = (roomNumbers) => {
    console.log(roomNumbers.unavialableDates);
    const isFound = roomNumbers.unavialableDates.some((date) =>
  ...
  return (
    <div className='reserve'>
      ...
      <FontAwesomeIcon
        ...
        {data.map(room => (
          <div key={room._id} className="rItem">
            <div className="rInfo">
              ...
              <div className="rMAx">Max Peaple: {room.maxPeople}</div>
              <div className="rPrice">Starting from {room.price}</div>
            </div>
        ...
        <button className='rButton' onClick={handleSubmit}>Reserve now</button>
      </div>
    </div>
  )
}
